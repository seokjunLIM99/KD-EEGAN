...
class CAM(nn.Module):
    ...
    def forward(self, x):
        ...
        output = torch.sigmoid(linear_max + linear_avg) * x
        return output
...
class SpatialPyramidPoolingWithAttention(nn.Module):
    def __init__(self, channels, levels=[1, 5, 10, 20], r=16):
        super(SpatialPyramidPoolingWithAttention, self).__init__()
    ...
    def forward(self, x):
        ...
        for level in self.levels:
            pool = F.adaptive_avg_pool2d(x, (level, level))
            pooled_features.append(pool.view(b, -1))

        spp_features = torch.cat(pooled_features, dim=-1)
...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.spp_at_t = SpatialPyramidPoolingWithAttention(channels=512)
        self.spp_at_t.cuda()

        self.spp_at_s = SpatialPyramidPoolingWithAttention(channels=512)
        self.spp_at_s.cuda()
        ...
        # wf2
        self.msoi = networks.CombinedPerceptualLoss_fft(cfg=r'C:\yolov9-c.yaml',weights=r'C:\best.pt')
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(list(self.netG_A.parameters()) + list(self.spp_at_t.parameters()) + list(self.spp_at_s.parameters()),lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        if self.opt.input_linear:
            self.real_A = (self.real_A - torch.min(self.real_A))/(torch.max(self.real_A) - torch.min(self.real_A))
        if self.opt.skip == 1:
            self.fake_B, self.latent_real_A, enc_first, enc_last, last_representation = self.netG_A.forward(self.real_img, self.real_A_gray)
        else:
    ...
    def backward_G(self, epoch, kd_loss):
        ...
        if self.opt.vgg > 0:
            ...
            self.msoi_loss = self.msoi.get_loss(fakeIm=self.fake_B, realIm=self.real_B) * 0.2
            ...
            self.loss_G = self.loss_G_A + self.loss_vgg_b*vgg_w + loss_kd + self.msoi_loss
    ...
    def optimize_parameters(self, epoch, kd_loss):
        # forward
        KDloss = kd_loss
    ...
    def get_current_errors(self, epoch):
        ...
        MSOI = self.msoi_loss.item() if hasattr(self, 'msoi_loss') else 0
    ...
    def save(self, label, save_dir):
        self.save_network(self.netG_A, 'G_A', label, self.gpu_ids, save_dir)
        self.save_network(self.netD_A, 'D_A', label, self.gpu_ids, save_dir)
        if self.opt.patchD:
            self.save_network(self.netD_P, 'D_P', label, self.gpu_ids, save_dir)
