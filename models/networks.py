...
from models.yolo import Model  # YOLOv9 모델 불러오기
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[], skip=False, opt=None):
    ...
    elif which_model_netG == 'sid_unet_resize_student':
        netG = Unet_resize_conv_student(opt, skip)
    elif which_model_netG == 'sid_unet_resize_student2':
        netG = Unet_resize_conv_student2(opt, skip)
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    elif which_model_netD == 'no_norm_4':# 현재 사용중
        netD = NoNormDiscriminator(input_nc, ndf, n_layers_D, use_sigmoid=use_sigmoid, gpu_ids=gpu_ids)
...
class Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        ...
        # conv8 block: Encoder의 conv2와 concat (채널 128 → 64)
    ...
    def forward(self, input, gray):
        ...
        return output, latent, enc_first, enc_last, last_representation

class Unet_resize_conv_student(nn.Module):
    def __init__(self, opt, skip):
        super(Unet_resize_conv_student, self).__init__()
        ...
        self.conv4_1 = nn.Conv2d(128, 512, 3, padding=p)
        self.LReLU4_1 = nn.LeakyReLU(0.2, inplace=True)
        ...
        self.deconv4 = nn.Conv2d(512, 128, 3, padding=p)
        self.conv5_1 = nn.Conv2d(256, 128, 3, padding=p)
        self.LReLU5_1 = nn.LeakyReLU(0.2, inplace=True)
        self.bn5_1 = nn.BatchNorm2d(128)
        self.conv5_2 = nn.Conv2d(128, 128, 3, padding=p)
        self.LReLU5_2 = nn.LeakyReLU(0.2, inplace=True)
        self.bn5_2 = nn.BatchNorm2d(128)

        self.deconv5 = nn.Conv2d(128, 64, 3, padding=p)
        self.conv6_1 = nn.Conv2d(128, 64, 3, padding=p)
        self.LReLU6_1 = nn.LeakyReLU(0.2, inplace=True)
        self.bn6_1 = nn.BatchNorm2d(64)
        self.conv6_2 = nn.Conv2d(64, 64, 3, padding=p)
        self.LReLU6_2 = nn.LeakyReLU(0.2, inplace=True)
        self.bn6_2 = nn.BatchNorm2d(64)

        self.deconv6 = nn.Conv2d(64, 32, 3, padding=p)
        self.conv7_1 = nn.Conv2d(64, 32, 3, padding=p)
        self.LReLU7_1 = nn.LeakyReLU(0.2, inplace=True)
        self.bn7_1 = nn.BatchNorm2d(32)
        self.conv7_2 = nn.Conv2d(32, 32, 3, padding=p)
        self.LReLU7_2 = nn.LeakyReLU(0.2, inplace=True)

        self.conv8 = nn.Conv2d(32, 3, 1)

    def forward(self, input, gray):
        input, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(input)
        gray, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(gray)

        gray_2 = F.avg_pool2d(gray, 2)
        gray_3 = F.avg_pool2d(gray_2, 2)
        gray_4 = F.avg_pool2d(gray_3, 2)

        x = self.bn1_1(self.LReLU1_1(self.conv1_1(torch.cat((input, gray), 1))))
        conv1 = self.bn1_2(self.LReLU1_2(self.conv1_2(x))) * gray
        x = self.max_pool1(conv1)
        enc_first = conv1

        x = self.bn2_1(self.LReLU2_1(self.conv2_1(x)))
        conv2 = self.bn2_2(self.LReLU2_2(self.conv2_2(x))) * gray_2
        x = self.max_pool2(conv2)

        x = self.bn3_1(self.LReLU3_1(self.conv3_1(x)))
        conv3 = self.bn3_2(self.LReLU3_2(self.conv3_2(x))) * gray_3
        x = self.max_pool3(conv3)

        x = self.bn4_1(self.LReLU4_1(self.conv4_1(x)))
        conv4 = self.bn4_2(self.LReLU4_2(self.conv4_2(x))) * gray_4
        enc_last = conv4

        conv4 = F.upsample(conv4, scale_factor=2, mode='bilinear')
        up5 = torch.cat([self.deconv4(conv4), conv3], 1)
        x = self.bn5_1(self.LReLU5_1(self.conv5_1(up5)))
        conv5 = self.bn5_2(self.LReLU5_2(self.conv5_2(x)))
        ...
        last_representation = conv7


        latent = self.conv8(conv7) * gray
...
class Unet_resize_conv_student2(nn.Module):
    def __init__(self, opt, skip):
        super(Unet_resize_conv_student2, self).__init__()
        ...
        self.conv2_1 = nn.Conv2d(32, 128, 3, padding=p)
        self.LReLU2_1 = nn.LeakyReLU(0.2, inplace=True)
        self.bn2_1 = nn.BatchNorm2d(128)
        self.conv2_2 = nn.Conv2d(128, 128, 3, padding=p)
        self.LReLU2_2 = nn.LeakyReLU(0.2, inplace=True)
        self.bn2_2 = nn.BatchNorm2d(128)
        self.max_pool2 = nn.MaxPool2d(2)

        self.conv3_1 = nn.Conv2d(128, 512, 3, padding=p)
        self.LReLU3_1 = nn.LeakyReLU(0.2, inplace=True)
        ...
        self.deconv3 = nn.Conv2d(512, 128, 3, padding=p)
        self.conv4_1 = nn.Conv2d(256, 128, 3, padding=p)
        self.LReLU4_1 = nn.LeakyReLU(0.2, inplace=True)
        ...
        self.deconv4 = nn.Conv2d(128, 32, 3, padding=p)
        self.conv5_1 = nn.Conv2d(64, 32, 3, padding=p)
        self.LReLU5_1 = nn.LeakyReLU(0.2, inplace=True)
        self.bn5_1 = nn.BatchNorm2d(32)
        self.conv5_2 = nn.Conv2d(32, 32, 3, padding=p)
        self.LReLU5_2 = nn.LeakyReLU(0.2, inplace=True)
    ...
    def forward(self, input, gray):
        ...
        conv3 = F.upsample(conv3, scale_factor=2, mode='bilinear')
        up4 = torch.cat([self.deconv3(conv3), conv2], 1)
        x = self.bn4_1(self.LReLU4_1(self.conv4_1(up4)))
        conv4 = self.bn4_2(self.LReLU4_2(self.conv4_2(x)))

        conv4 = F.upsample(conv4, scale_factor=2, mode='bilinear')
        up5 = torch.cat([self.deconv4(conv4), conv1], 1)
        x = self.bn5_1(self.LReLU5_1(self.conv5_1(up5)))
        conv5 = self.LReLU5_2(self.conv5_2(x))

        latent = self.conv6(conv5) * gray
...
class YOLOv9PerceptualLoss_fft:
    def __init__(self, cfg=r'C:',weights=r'C:'):
        self.model = Model(cfg=cfg).eval()
    ...
    def extract_features(self, x):
        ...
        features.append(x)
        x_fft1 = self.FFT2(x)
        features_fft.append(x_fft1)

        x = self.model.model[4:6](x)
        features.append(x)
        x_fft2 = self.FFT2(x)
        features_fft.append(x_fft2)

        x = self.model.model[6:8](x)
        features.append(x)
        x_fft3 = self.FFT2(x)
        features_fft.append(x_fft3)

        x = self.model.model[8:10](x)
        features.append(x)
        x_fft4 = self.FFT2(x)
        features_fft.append(x_fft4)

        return features_fft, features

class YOLOv9PerceptualLoss:
    def __init__(self, cfg=r'C',weights=r'C'):
...
class MSOIFFT:
    def __init__(self, cfg, weights, loss_fn=nn.L1Loss()):
        self.yolo = YOLOv9PerceptualLoss_fft(cfg, weights)
    ...
    def get_loss(self, fakeIm, realIm):
        ...
        for i in range(fakeIm.size(0)):
            fakeIm[i, :, :, :] = self.transform(fakeIm[i, :, :, :])
            realIm[i, :, :, :] = self.transform(realIm[i, :, :, :])

        f_fake_features_fft, f_fake_features = self.yolo.extract_features(fakeIm)
        f_real_features_fft, f_real_features = self.yolo.extract_features(realIm)

        msoi_fft_loss = 0
        for f_fake, f_real in zip(f_fake_features_fft, f_real_features_fft):
            f_real_no_grad = f_real.detach()
            msoi_fft_loss += self.loss_fn(f_fake, f_real_no_grad)

        msoi_loss = 0
        for f_fake, f_real in zip(f_fake_features, f_real_features):
            f_real_no_grad = f_real.detach()
            msoi_loss += self.loss_fn(f_fake, f_real_no_grad)

        msoi_loss = msoi_loss * 0.1

        pixel_loss = nn.MSELoss()(fakeIm, realIm)

        return 0.1 * msoi_loss + 0.1 * msoi_fft_loss + 0.5 * pixel_loss

    def __call__(self, fakeIm, realIm):
        return self.get_loss(fakeIm, realIm)


class CombinedPerceptualLoss_fft:
    def __init__(self, cfg, weights, msoi_weight=1.0, msoi_fft_weight=1.0):
        print(weights)
        self.yolo_loss_fft = MSOIFFT(cfg, weights)
        self.msoi_fft_lambda = msoi_fft_weight

    def get_loss(self, fakeIm, realIm):
        yolo_loss_value_fft = self.yolo_loss_fft(fakeIm, realIm)

        combined_loss = self.msoi_fft_lambda * yolo_loss_value_fft
