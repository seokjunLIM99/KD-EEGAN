...
class PairDataset(BaseDataset):
    def initialize(self, opt):
        ...
        self.dir_A = r'D:\dataset\SmokeDetector\lq1\train\images'
        self.dir_B = r'D:\dataset\SmokeDetector\gt\train\images'
        ...
        if self.A_size != self.B_size:
            raise RuntimeError(f"Mismatch in dataset sizes: {self.A_size} (A) vs {self.B_size} (B)")

        # 1:1 대응 검증 (파일명 비교)
        for a_path, b_path in zip(self.A_paths, self.B_paths):
            if os.path.basename(a_path) != os.path.basename(b_path):
                raise RuntimeError(f"File name mismatch: {os.path.basename(a_path)} != {os.path.basename(b_path)}")
